...
export default class PuzzleGameScene extends Phaser.Scene {
	...
	createPreview() {
		...
		this.previewGrid = Array(3)
			.fill()
			.map(() => Array(3).fill(null)) // 2D array for 3x3 preview grid
		...
		for (var row = 0; row < 3; row++) {
			for (var col = 0; col < 3; col++) {
				if (index < 8) {
					...
					var tile = this.add.image(x, y, key).setDisplaySize(this.previewSize, this.previewSize).setOrigin(0)
